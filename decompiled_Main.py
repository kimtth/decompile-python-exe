...
import numpy as np, cv2, socket, os, io, time, imghdr, sys
...
class mywindow(QMainWindow, Ui_Client):
        ...
        if event.key() == Qt.Key_W:
            ...
            if event.key() == Qt.Key_W and not event.isAutoRepeat():
                ...
                if event.key() == Qt.Key_A:
                    ...
                    if not event.isAutoRepeat() or self.Key_Q is True:
                        self.on_btn_Stop()
                        self.Key_Q = False
                elif event.key() == Qt.Key_E:
                    if not event.isAutoRepeat() or self.Key_E is True:
                        self.on_btn_Stop()
                        self.Key_E = False
                elif event.key() == Qt.Key_Z:
                    if not event.isAutoRepeat() or self.Key_Z is True:
                        self.on_btn_Stop()
                        self.Key_Z = False
                elif event.key() == Qt.Key_X and not event.isAutoRepeat():
                    if self.Key_X is True:
                        self.on_btn_Stop()
                        self.Key_X = False
                if event.key() == Qt.Key_Space:
                    if not event.isAutoRepeat():
                        if self.Key_Space is True:
                            self.on_btn_Buzzer()
                            self.Key_Space = False
    ...
    def LedChange(self, b):
        ...
        if b.text() == "Led2":
            self.led_Index = str(2)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
        if b.text() == "Led3":
            self.led_Index = str(4)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
        if b.text() == "Led4":
            self.led_Index = str(8)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
        if b.text() == "Led5":
            self.led_Index = str(16)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
        if b.text() == "Led6":
            self.led_Index = str(32)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
        if b.text() == "Led7":
            self.led_Index = str(64)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
        if b.text() == "Led8":
            self.led_Index = str(128)
            if b.isChecked() is True:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + color)
            else:
                self.TCP.sendData(cmd.CMD_LED + self.intervalChar + self.led_Index + led_Off)
                ...
                for oneCmd in cmdArray:
                    ...
                    if cmd.CMD_SONIC in Massage:
                        ...
                        if cmd.CMD_POWER in Massage:
                            percent_power = int((float(Massage[1]) - 7) / 1.4 * 100)
    ...
    def find_Face(self, face_x, face_y):
        if face_x != 0:
            if face_y != 0:
                ...
                if offset_x > -0.15and offset_y > -0.15 and offset_y > -0.15 and offset_y < 0.15:
                    pass
                else:
                    self.HSlider_Servo1.setValue(self.servo1)
                    self.VSlider_Servo2.setValue(self.servo2)
